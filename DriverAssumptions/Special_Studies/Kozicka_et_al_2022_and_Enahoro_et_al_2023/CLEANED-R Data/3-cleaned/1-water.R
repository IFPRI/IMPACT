...
wu_meat<-round(wr_sum/(meat),0) #num cow here takes the average of the pastoral cows over the year
...
names(water_ind2)<-c("total water consumption","difference between water consumption and rainfall", 'water consumption per cow', 'water consumption per ton of milk', "water consumption per ton of meat", "average water consumption intensity")
...
water_ind2$evaluation <- ifelse(water_ind2$percent <= water_ind_val$peu, 'low', ifelse((water_ind2$percent >= water_ind_val$peu) & (water_ind2$percent <= water_ind_val$peu),'medium','high'))
...
par(mfrow=c(1,2),mar=c(1, 2.25, 1, 3))
